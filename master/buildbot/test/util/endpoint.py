...
class EndpointMixin(interfaces.InterfaceTests):
    ...
    def setUpEndpoint(self, needDB=True):
        ...
        self.rt = self.endpointClass(self.master)

        self.assertIsInstance(self.rt.pathPattern, tuple,
                "did you forget the ',' in a 1-element pathPattern?")
        self.path_args = set([
            arg[1:] for arg in self.rt.pathPattern
            if arg.startswith(':') ])
    ...
    def callGet(self, options, kwargs):
        ...
        return self.rt.get(options, kwargs)

    # interface tests

    def test_get_spec(self):
        @self.assertArgSpecMatches(self.rt.get)
        def get(self, options, kwargs):
            pass

    def test_control_spec(self):
        @self.assertArgSpecMatches(self.rt.control)
